...
internal sealed class BoundTypeClause : BoundNode {
    ...
    /// <param name="type">The language type, not the <see cref="Node" /> type.</param>
    ...
    internal BoundTypeClause(
        TypeSymbol type, bool isImplicit = false, bool isConstantReference = false, bool isReference = false,
    ...
    /// <summary>
    /// If the <see cref="BoundTypeClause.type" />, <see cref="BoundTypeClause.isReference" />,
    ...
    internal static BoundTypeClause Copy(BoundTypeClause typeClause) {
        return new BoundTypeClause(
            typeClause.type, typeClause.isImplicit, typeClause.isConstantReference, typeClause.isReference,
    ...
    internal static BoundTypeClause Reference(BoundTypeClause typeClause) {
        return new BoundTypeClause(
            typeClause.type, typeClause.isImplicit, false, true, typeClause.isConstant,
    ...
    internal BoundTypeClause ChildType() {
        if (dimensions > 0)
            return new BoundTypeClause(
                type, isImplicit, isConstantReference, isReference, isConstant, isNullable, isLiteral, dimensions - 1);
        else
            return null;
    }
    ...
    internal BoundTypeClause BaseType() {
        if (dimensions > 0)
            return new BoundTypeClause(
                type, isImplicit, isConstantReference, isReference, isConstant, isNullable, isLiteral, 0);
        else
            return this;
    }
}
