...
[DebuggerTypeProxy(typeof(ArrayBuilder2<>.DebuggerProxy))]
internal sealed partial class ArrayBuilder2<T> : IReadOnlyCollection<T>, IReadOnlyList<T>, ICollection<T> {
    /// <summary>
    ...
    private sealed class DebuggerProxy {
        ...
        public DebuggerProxy(ArrayBuilder2<T> builder) {
            _builder = builder;
        }
    ...
    private readonly ObjectPool<ArrayBuilder2<T>>? _pool;

    public ArrayBuilder2(int size) {
        _builder = ImmutableArray.CreateBuilder<T>(size);
    }

    public ArrayBuilder2()
        : this(8) { }

    private ArrayBuilder2(ObjectPool<ArrayBuilder2<T>> pool)
        : this() {
        _pool = pool;
    }
    ...
    public ImmutableArray<U> ToDowncastedImmutable<U>()
        ...
        var tmp = ArrayBuilder2<U>.GetInstance(Count);
    ...
    private static readonly ObjectPool<ArrayBuilder2<T>> s_poolInstance = CreatePool();
    public static ArrayBuilder2<T> GetInstance() {
        var builder = s_poolInstance.Allocate();
        Debug.Assert(builder.Count == 0);
        return builder;
    }

    public static ArrayBuilder2<T> GetInstance(int capacity) {
        var builder = GetInstance();
        builder.EnsureCapacity(capacity);
        return builder;
    }

    public static ArrayBuilder2<T> GetInstance(int capacity, T fillWithValue) {
        var builder = GetInstance();
        builder.EnsureCapacity(capacity);
    ...
    public static ObjectPool<ArrayBuilder2<T>> CreatePool() {
        return CreatePool(128); // we rarely need more than 10
    }

    public static ObjectPool<ArrayBuilder2<T>> CreatePool(int size) {
        ObjectPool<ArrayBuilder2<T>>? pool = null;
        pool = new ObjectPool<ArrayBuilder2<T>>(() => new ArrayBuilder2<T>(pool!), size);
        return pool;
    }
    ...
    internal Dictionary<K, ImmutableArray<T>> ToDictionary<K>(Func<T, K> keySelector, IEqualityComparer<K>? comparer = null)
        ...
        var accumulator = new Dictionary<K, ArrayBuilder2<T>>(Count, comparer);
        for (var i = 0; i < Count; i++) {
            var item = this[i];
            var key = keySelector(item);
            if (!accumulator.TryGetValue(key, out var bucket)) {
                bucket = ArrayBuilder2<T>.GetInstance();
    ...
    public void AddRange(ArrayBuilder2<T> items) {
        _builder.AddRange(items._builder);
    }

    public void AddRange<U>(ArrayBuilder2<U> items, Func<U, T> selector) {
        foreach (var item in items) {
            _builder.Add(selector(item));
        }
    }

    public void AddRange<U>(ArrayBuilder2<U> items) where U : T {
        _builder.AddRange(items._builder);
    }

    public void AddRange<U>(ArrayBuilder2<U> items, int start, int length) where U : T {
    ...
    public ImmutableArray<S> SelectDistinct<S>(Func<T, S> selector) {
        var result = ArrayBuilder2<S>.GetInstance(Count);
