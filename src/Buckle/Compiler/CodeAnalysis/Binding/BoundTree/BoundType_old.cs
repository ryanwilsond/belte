...
internal sealed class BoundType : BoundExpression
{
        ...
        ImmutableArray<BoundExpression>? sizes = null)
    {
        ...
        this.templateArguments = templateArguments ?? ImmutableArray<TypeOrConstant>.Empty;
        this.arity = arity;
    ...
    internal static BoundType Assume(object value)
    {
        ...
        if (value is BoundType) return Type;
        if (value is null) return new BoundType(null, isNullable: true);
    ...
    internal static BoundType Compound(ImmutableArray<TypeOrConstant> templateArguments, BoundType type)
    {
        if (type.typeSymbol is TemplateParameterSymbol t && templateArguments.Length > 0)
        {
            var resolvedType = templateArguments[t.parameter.ordinal - 1].type;
    ...
    /// </summary>
    internal static BoundType Clarify(BoundType type, Dictionary<ParameterSymbol, TypeOrConstant> templates)
    {
        if (type.templateArguments.Length == 0)
            return type;

        var builder = ImmutableArray.CreateBuilder<TypeOrConstant>();

        foreach (var argument in type.templateArguments)
        {
            if (argument.type.typeSymbol is TemplateParameterSymbol t)
            {
                if (templates.TryGetValue(t.parameter, out var typeOrConstant))
                {
    ...
    internal bool Equals(BoundType type, bool loose = false, bool isTypeCheck = false)
    {
        ...
        if (typeSymbol is TemplateParameterSymbol lt && type?.typeSymbol is TemplateParameterSymbol rt)
        {
