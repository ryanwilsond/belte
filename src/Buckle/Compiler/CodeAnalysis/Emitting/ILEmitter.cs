...
#pragma warning restore IL3000
    ...
    internal TypeReference GetType(TypeSymbol type, bool byRef = false, bool import = true) {
        ...
        TypeReference GetTypeWithContainingGenerics(NamedTypeSymbol type) {
            ...
            if (allTypeArgs.Count > 0) {
                ...
                _assemblyDefinition.MainModule.ImportReference(Resolve(typeReference));
                return typeReference;
            }
    ...
    private TypeReference GetFuncType(FunctionMethodSymbol signature) {
        if (signature.returnsVoid && signature.parameterCount == 0) {
            ...
            _assemblyDefinition.MainModule.ImportReference(Resolve(typeRef));
            return typeRef;
            ...
            _assemblyDefinition.MainModule.ImportReference(Resolve(genericRef));
            return genericRef;
        } else {
    ...
    internal MethodReference GetMethod(MethodSymbol method) {
        ...
        if (found) {
            ...
            if (method.arity > 0) {
                var def = _assemblyDefinition.MainModule.ImportReference(Resolve(value));
    ...
    private TypeReference ResolveType(PENamedTypeSymbol typeSymbol) {
        ...
        _assemblyDefinition.MainModule.ImportReference(Resolve(currentFoundType));
    ...
    private void GenerateSTLMap() {
        if (_program.compilation.options.noStdLib) {
            ...
            _stlMap = new Dictionary<string, MethodReference>() {
                ...
                { "String_Replacement_SSS", ResolveMethod("Belte.Runtime.Utilities", "StringReplace", ["System.String", "System.String", "System.String"]) },
