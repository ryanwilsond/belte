...
internal partial class SyntaxTreeSemanticModel : PublicSemanticModel {
    /*
    private static readonly Func<BelteSyntaxNode, bool> IsMemberDeclarationFunction = IsMemberDeclaration;

    private readonly Compilation _compilation;
    private readonly SyntaxTree _syntaxTree;

    private ImmutableDictionary<BelteSyntaxNode, MemberSemanticModel> _memberModels =
        ImmutableDictionary<BelteSyntaxNode, MemberSemanticModel>.Empty;

    private readonly BinderFactory _binderFactory;
    private Func<BelteSyntaxNode, MemberSemanticModel> _createMemberModelFunction;
    private readonly SemanticModelOptions _options;

    internal SyntaxTreeSemanticModel(Compilation compilation, SyntaxTree syntaxTree, SemanticModelOptions options) {
        _compilation = compilation;
        _syntaxTree = syntaxTree;
        _options = options;
        _binderFactory = compilation.GetBinderFactory(
            this.syntaxTree,
            (options & SemanticModelOptions.IgnoreAccessibility) != 0
        );
    }
    ...
    internal SyntaxTreeSemanticModel(
        ...
        SyntaxTree speculatedSyntaxTree,
        SemanticModelOptions options) {
        _compilation = parentCompilation;
        _syntaxTree = speculatedSyntaxTree;
        _binderFactory = _compilation.GetBinderFactory(
            parentSyntaxTree,
            ignoreAccessibility: (options & SemanticModelOptions.IgnoreAccessibility) != 0
        );
        _options = options;
    }
    ...
    internal override BelteSyntaxNode root => (BelteSyntaxNode)_syntaxTree.GetRoot();
    ...
    private void VerifySpanForGetDiagnostics(TextSpan span) {
        if (span is not null && !root.fullSpan.Contains(span))
            throw new ArgumentException("span");
    }

    public override ImmutableArray<BelteDiagnostic> GetSyntaxDiagnostics(
        TextSpan span = null,
    ...
    public override ImmutableArray<BelteDiagnostic> GetDeclarationDiagnostics(
        TextSpan span = null,
        CancellationToken cancellationToken = default) {
        VerifySpanForGetDiagnostics(span);
        return Compilation.GetDiagnosticsForSyntaxTree(
    ...
    public override ImmutableArray<BelteDiagnostic> GetMethodBodyDiagnostics(
        TextSpan span = null,
        CancellationToken cancellationToken = default) {
        VerifySpanForGetDiagnostics(span);
        return Compilation.GetDiagnosticsForSyntaxTree(
    ...
    public override ImmutableArray<BelteDiagnostic> GetDiagnostics(
        TextSpan span = null,
        CancellationToken cancellationToken = default) {
        VerifySpanForGetDiagnostics(span);
        return Compilation.GetDiagnosticsForSyntaxTree(
    ...
    internal override Binder GetEnclosingBinderInternal(int position) {
        ...
        return _binderFactory.GetBinder((BelteSyntaxNode)token.parent, position)
            .WithAdditionalFlags(GetSemanticModelBinderFlags());
    }

    internal override IOperation GetOperationWorker(BelteSyntaxNode node, CancellationToken cancellationToken) {
    ...
    internal override SymbolInfo GetSymbolInfoWorker(BelteSyntaxNode node, SymbolInfoOptions options, CancellationToken cancellationToken = default(CancellationToken)) {
    ...
    internal override CSharpTypeInfo GetTypeInfoWorker(BelteSyntaxNode node, CancellationToken cancellationToken = default(CancellationToken)) {
        // in case this is right side of a qualified name or member access (or part of a cref)
        node = SyntaxFactory.GetStandaloneNode(node);
    ...
    private Symbol GetSemanticInfoSymbolInNonMemberContext(BelteSyntaxNode node, bool bindVarAsAliasFirst) {
    ...
    internal override ImmutableArray<Symbol> GetMemberGroupWorker(BelteSyntaxNode node, SymbolInfoOptions options, CancellationToken cancellationToken = default(CancellationToken)) {
        // in case this is right side of a qualified name or member access (or part of a cref)
        node = SyntaxFactory.GetStandaloneNode(node);
    ...
    internal override ImmutableArray<IPropertySymbol> GetIndexerGroupWorker(BelteSyntaxNode node, SymbolInfoOptions options, CancellationToken cancellationToken = default(CancellationToken)) {
        // in case this is right side of a qualified name or member access (or part of a cref)
        node = SyntaxFactory.GetStandaloneNode(node);
    ...
    internal override Optional<object> GetConstantValueWorker(BelteSyntaxNode node, CancellationToken cancellationToken) {
    ...
    internal override Conversion ClassifyConversionForCast(ExpressionSyntax expression, TypeSymbol destination) {
        ...
        var model = GetMemberModel(expression);
    ...
    public override BelteSemanticModel parentModel => null;
    ...
    private MemberSemanticModel GetMemberModel(int position) {
        AssertPositionAdjusted(position);
        BelteSyntaxNode node = (BelteSyntaxNode)Root.FindTokenIncludingCrefAndNameAttributes(position).Parent;
        BelteSyntaxNode memberDecl = GetMemberDeclaration(node);
    ...
    /// </summary>
    internal ImmutableDictionary<BelteSyntaxNode, MemberSemanticModel> TestOnlyMemberModels => _memberModels;
    ...
    private static BelteSyntaxNode GetMemberDeclaration(SyntaxNode node) {
        return node.FirstAncestorOrSelf(IsMemberDeclarationFunction);
    }

    private MemberSemanticModel GetOrAddModelIfContains(BelteSyntaxNode node, TextSpan span) {
    ...
    private MemberSemanticModel GetOrAddModel(BelteSyntaxNode node) {
    ...
    internal MemberSemanticModel GetOrAddModel(BelteSyntaxNode node, Func<BelteSyntaxNode, MemberSemanticModel> createMemberModelFunction) {
    ...
    private MemberSemanticModel CreateMemberModel(BelteSyntaxNode node) {
        ...
        MemberSemanticModel createMethodBodySemanticModel(BelteSyntaxNode memberDecl, SourceMemberMethodSymbol symbol) {
            ExecutableCodeBinder binder = symbol?.TryGetBodyBinder(_binderFactory, this.IgnoresAccessibility);
    ...
    private static bool IsMemberDeclaration(BelteSyntaxNode node) {
    ...
    private NamedTypeSymbol GetDeclaredNamedType(BelteSyntaxNode declarationSyntax, string name) {
        Debug.Assert(declarationSyntax != null);
    ...
    private NamespaceOrTypeSymbol GetDeclaredNamespaceOrType(BelteSyntaxNode declarationSyntax) {
    ...
    private string GetDeclarationName(BelteSyntaxNode declaration) {
    ...
    private string GetDeclarationName(BelteSyntaxNode declaration, ExplicitInterfaceSpecifierSyntax explicitInterfaceSpecifierOpt, string memberName) {
    ...
    private NamespaceOrTypeSymbol GetDeclaredTypeMemberContainer(BelteSyntaxNode memberDeclaration) {
    ...
    private Symbol GetDeclaredMemberSymbol(BelteSyntaxNode declarationSyntax) {
        CheckSyntaxNode(declarationSyntax);
