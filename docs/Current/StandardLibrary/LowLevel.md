...
| `int! Length<type TElem>(Buffer<TElem>!)` | Gets the length of the given array, or 0 if not passed an array. |
| `void Sort<type TElem>(Buffer<TElem>!)` | Sorts the given array, or does nothing if not passed an array. |
| `void Fill<type TElem>(Buffer<TElem>!, TElem!)` | Fills the given array with the given value. |
